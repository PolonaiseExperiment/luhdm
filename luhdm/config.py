...
Q_THRESH = 1e3  # Momentum / analysis threshold, GeV/c (natural units; 1 TeV). Raised from 1e2 for the v7 analysis window: 1 TeV sits near the median of the impulse-efficiency turn-on (eps ~ 0.53) rather than out on its cliff, so the window edge is a stated physics threshold instead of a place where the limit is carried by a steeply-varying, poorly-constrained efficiency. All 8 mode-1 candidates (1.52-12.8 TeV) remain inside; the 2 sub-TeV mode-2 impulses (554, 946 GeV) fall outside and are dropped by the optimum-interval support window automatically. Sets the kinematic wall at m_wall = Q_THRESH / VESC = 5.51e5 GeV.
