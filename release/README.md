...
| `luhdm_datarelease_v10_B_f0p1_noatm.h5` | 0.58 MB | **Dataset B.** `f_DM = 0.1`, atmospheric propagation **off**. The plane the composite cross-section benchmark is quoted on. |
...
| `aux/null_calibration_toymc.json` | 16 kB | **Null-distribution calibration.** The toy Monte Carlo behind the transient statistic's null: the exceedance curve the selection threshold is read off, checksummed here so the claim in the Letter is checkable rather than asserted. |
...
The last line of the block above is the historical bridge: `N_req = 9.35` puts
the cut at 2.70 × 10¹⁴ GeV, the right-hand edge that earlier, cap-truncated
versions of this analysis closed at. So the two schemes describe the same edge
under different standards, and this release makes the standard visible instead
of burying it in an integration limit. Note that the bridging value moves with
the halo frame: `m_cut` scales with ⟨v⟩, so the rest-frame builds needed
`N_req = 6.8` to land on the same anchor and the lab-frame build needs 9.35.
Neither is a measurement — 3 is the convention this release ships.
